...
(function($env){
    ...
    $env.loadLocalScript = function(script, parser){
        ...
        var types, type, src, i, base, source, loc, fileReg = /^file:\/\//
            docWrites = [],
                ...
                for(i=0;i<types.length;i++){
                    if($env.scriptTypes[types[i]]){
						if(script.src){
							...
							loc = $env.location(script.src.match(/([^\?#]*)/)[1], base )
							var source = fileReg.test(loc) ? readFile(loc.replace(fileReg,"")) : readUrl(loc)
							script.nodeValue = source
                            load(loc);
                            //lets you register a function to execute 
